...
def tsv_to_df_generator(dir_path):
    for f in dir_path.glob('*.tsv'):
        ...
        # Only cell 9 (centre cell) is reduced
        rcp_cells = [9]
...
def get_seed_stats(df, cell_set: list, sleep_fix, rcp_cells: list):
    ...
    # If the throughput series contain nan values, replace them with 0.0
    tp_series = df.loc[df['serving_cell_id'].isin(cell_set), 'ue_throughput(Mb/s)']
    tp_series = tp_series.fillna(0)
    ...
    ##### START COPY HERE #######
    #############################
    # Get the total number of unique users in the cell_set
    U = len(df.loc[df['serving_cell_id'].isin(cell_set), 'ue_id'].unique())

    # Adjust for rows where no users are attached
    for cell in cell_set:
        if cell in rcp_cells:
            # For rows where the 'ue_id' value is nan, minus 1 from U
            for i in range(len(df.loc[(df['serving_cell_id'] == cell) & (df['ue_id'].isna())])):
                U -= 1

    # Get the number of cells in the cell_set
    K = len(cell_set)
    ...
    if U != 0:
        mean_user_throughput = T / U
    else:
        mean_user_throughput = 0
...
df_cells_all_stats_per_power_level_dBm.to_feather(
    f'{project_path_str}/data/analysis/GLOBECOM/centre_scenario_all_cells.feather'
)
...
df_cells_power_reduced_stats_per_power_level_dBm.to_feather(
    f'{project_path_str}/data/analysis/GLOBECOM/centre_scenario_variable_cells.feather'
)
...
df_cells_power_static_stats_per_power_level_dBm.to_feather(
    f'{project_path_str}/data/analysis/GLOBECOM/centre_scenario_static_cells.feather'
)
...
fig_filename = f'{date_str}_rcp_cell_{reduced_cells_str}_power_dBm_v3_0.pdf'
